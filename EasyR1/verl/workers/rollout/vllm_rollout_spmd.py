...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        for q_idx, ans_list in enumerate(full_answers_per_q):
            for ans_idx, ans in enumerate(ans_list):
                for r in ratios:
                    prefix = ans[:max(1, int(len(ans) * r))] + add_info
                    full_input_ids = list(batch_raw_prompt_ids[q_idx]) + tokenizer(prefix, add_special_tokens=False)["input_ids"]
        ...
        for q_idx, ans_list in enumerate(full_answers_per_q):
            ...
            for idx, (p_idx, a_idx, prefix) in enumerate(prefix_map):
                if p_idx == q_idx:
                    cont_text = extract_boxed_sentence(all_conts[idx].outputs[0].text)
                    combined_q.append(prefix + cont_text)
            assert len(combined_q) == n_total, f"每个问题生成数量 {len(combined_q)} != {n_total}"
            final_answers.extend(combined_q)
    ...
    @torch.no_grad()
    def generate_val_sequences(self, prompts: DataProto) -> DataProto:
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            full_answers_per_q = []
            for completion in completions:  # completion 对应一个 prompt
                answers = [output.text for output in completion.outputs]
                full_answers_per_q.append(answers)
                # print(f"!!生成的完整答案:{len(answers)} {answers[0]} {len(answers[0])}")
